...
class MarketScanner:
    ...
    def get_target_universe(self, max_pairs: int = config.MAX_PAIRS) -> List[str]:
        """
        Screens for liquid, pure-crypto altcoins while blacklisting non-ASCII meme tokens
        and BTC-slaved dinosaur assets.
        """
        try:
        ...
        logger.info(f"Target Universe selected ({len(selected)} pairs): {selected}")
        return selected
